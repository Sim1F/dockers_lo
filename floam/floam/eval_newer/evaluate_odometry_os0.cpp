...
void plotPathPlot (string dir,vector<int32_t> &roi,string folder_name_str) {

  // gnuplot file name
  char command[1024];
  // char file_name[256];
  // sprintf(file_name,"%02d.gp",idx);
  string full_name = dir + "/" + folder_name_str;
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    fprintf(fp,"plot \"%s_poses_kitti.txt\" using 1:2 lc rgb \"#FF0000\" title 'Ground Truth' w lines,",folder_name_str.c_str());
    fprintf(fp,"\"%s_poses_kitti.txt\" using 3:4 lc rgb \"#0000FF\" title 'LiDAR Odometry' w lines,",folder_name_str.c_str());
    fprintf(fp,"\"< head -1 %s_poses_kitti.txt\" using 1:2 lc rgb \"#000000\" pt 4 ps 1 lw 2 title 'Sequence Start' w points\n",folder_name_str.c_str());
    
    // close file
    fclose(fp);
    
    // run gnuplot => create png + eps
    sprintf(command,"cd %s; gnuplot %s",dir.c_str(),folder_name_str.c_str());
    system(command);
  }
  
  // create pdf and crop
  sprintf(command,"cd %s; ps2pdf %s.eps %s_large.pdf",dir.c_str(),folder_name_str.c_str(),folder_name_str.c_str());
  system(command);
  sprintf(command,"cd %s; pdfcrop %s_large.pdf %s.pdf",dir.c_str(),folder_name_str.c_str(),folder_name_str.c_str());
  system(command);
  sprintf(command,"cd %s; rm %s_large.pdf",dir.c_str(),folder_name_str.c_str());
  system(command);
}
...
// int training_set_minmax[] = {0,7}; // without stairs
int training_set_minmax[] = {0,8}; // with stairs
// int testing_set_minmax[] = {11,22};

bool eval (string abs_path_to_data) {

  // ground truth and result directories
  // abs_path_to_data = "/media/simone/PortableSSD/datasets/os0_newer_college";
  // string path_to_data = abs_path_to_data + "/dataset/sequences";
  ...
  // /media/simone/PortableSSD/datasets/os0_newer_college

  // /media/simone/PortableSSD/datasets/os0_newer_college/floam/catacombs_easy/catacombs_easy_poses_kitti.txt

  // /media/simone/PortableSSD/datasets/os0_newer_college/catacombs_easy/gt_kitti.txt
  ...
  for (int32_t i=training_set_minmax[0]; i<training_set_minmax[1]; i++) {
    string folder_name_str = "";
    switch (i) {
      case 0:
        folder_name_str = "catacombs_easy";
        break;
      case 1:
        folder_name_str = "catacombs_medium";
        break;
      case 2:
        folder_name_str = "cloister";
        break;
      case 3:
        folder_name_str = "math_easy";
        break;
      case 4:
        folder_name_str = "math_medium";
        break;
      case 5:
        folder_name_str = "quad_easy";
        break;
      case 6:
        folder_name_str = "quad_medium";
        break;
      case 7:
        folder_name_str = "stairs";
        break;
      default:
        folder_name_str = "";
        break;
    ...
    saveSequenceErrors(seq_err, error_dir + "/" + estimate_name);
    ...
    savePathPlot(poses_gt,poses_result,plot_path_dir + "/" + estimate_name);
    vector<int32_t> roi = computeRoi(poses_gt,poses_result);
    plotPathPlot(plot_path_dir,roi,folder_name_str);

    // save + plot individual errors
    saveErrorPlots(seq_err,plot_error_dir,folder_name);
    plotErrorPlots(plot_error_dir,folder_name);

    saveStats(seq_err,result_dir,folder_name);
