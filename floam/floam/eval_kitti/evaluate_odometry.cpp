...
void plotPathPlot (string dir,vector<int32_t> &roi,int32_t idx) {
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    fprintf(fp,"plot \"%02d_poses_kitti.txt\" using 1:2 lc rgb \"#FF0000\" title 'Ground Truth' w lines,",idx);
    fprintf(fp,"\"%02d_poses_kitti.txt\" using 3:4 lc rgb \"#0000FF\" title 'LiDAR Odometry' w lines,",idx);
    fprintf(fp,"\"< head -1 %02d_poses_kitti.txt\" using 1:2 lc rgb \"#000000\" pt 4 ps 1 lw 2 title 'Sequence Start' w points\n",idx);
...
int training_set_minmax[] = {0,11};
int testing_set_minmax[] = {11,22};

bool eval (string abs_path_to_data) {

  // ground truth and result directories
  // abs_path_to_data = "/media/simone/PortableSSD/datasets/kitti";
  ...
  // /media/simone/PortableSSD/datasets/kitti

  // /media/simone/PortableSSD/datasets/kitti/floam/00/00_poses_kitti.txt

  // /media/simone/PortableSSD/datasets/kitti/data_odometry_velodyne/dataset/sequences/00/00.txt
  ...
  for (int32_t i=training_set_minmax[0]; i<training_set_minmax[1]; i++) {
    ...
    string gt_path = abs_path_to_data + "/data_odometry_velodyne/dataset/sequences/" + folder_name_str + "/" + folder_name_str + ".txt";
