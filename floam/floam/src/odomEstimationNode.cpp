...
void odom_estimation(){
    std::cout << "ODOM ESTIMATION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    std::ofstream os;
    // os.open("/catkin_ws/src/floam/results/odometry_new.txt");
    // os.open("/home/odometry_new.txt");
    os.open(odomEstimation.path_to_output);
    ...
    while(1){
        if(!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty()){
            ...
            // timestamp + tum format
            // os << std::fixed << std::setprecision(12) << pointcloud_time << " ";
            // os << t_current.x() << " " << t_current.y() << " " << t_current.z() << " ";
            // os << q_current.x() << " " << q_current.y() << " " << q_current.z() << " " << q_current.w() << std::endl;

            // kitti format
            const Eigen::Isometry3d base_to_world = Eigen::Translation3d(t_current) * Eigen::Quaterniond(q_current);
            for (int row = 0; row < 3; row++) {
                for(int col = 0; col < 4; col++) {
                    os << base_to_world(row,col) << " ";
                }
            }
            os << std::endl;
...
int main(int argc, char **argv)
{
    ...
    nh.getParam("/path_to_output", path_to_output);
    ...
    odomEstimation.path_to_output = path_to_output;
