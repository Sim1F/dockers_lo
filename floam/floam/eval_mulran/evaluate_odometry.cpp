...
// vector<Matrix> loadPosesGt(string file_name) {
//   vector<Matrix> poses;
//   FILE *fp = fopen(file_name.c_str(), "r");
//   if (!fp)
//     return poses;
...
//     Matrix P = Matrix::eye(4);
//     char *token = strtok(line, ","); // Split the line into tokens using comma as delimiter

//     // Skip the first token (timestamp)
//     token = strtok(NULL, ","); // Move to the next token

//     if (token) { // Check if there is a valid token
//       for (int i = 0; i < 12; ++i) {
//         if (token) {
//           P.val[i / 4][i % 4] = atof(token); // Convert token to float and assign to matrix
//           token = strtok(NULL, ",");
//         }
//       }
//       poses.push_back(P);
...
void plotPathPlot (string dir,vector<int32_t> &roi,string city,int32_t idx) {
  ...
  char idx_char[256];
  sprintf(idx_char,"%02d",idx);
  string result_name = city + "_" + idx_char;
  string result_name_txt = result_name + ".txt";
  string result_name_gp = result_name + ".gp";
  string full_name = dir + "/" + result_name_gp;
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    fprintf(fp,"plot \"%s\" using 1:2 lc rgb \"#FF0000\" title 'Ground Truth' w lines,",result_name_txt.c_str());
    fprintf(fp,"\"%s\" using 3:4 lc rgb \"#0000FF\" title 'LiDAR Odometry' w lines,",result_name_txt.c_str());
    fprintf(fp,"\"< head -1 %s\" using 1:2 lc rgb \"#000000\" pt 4 ps 1 lw 2 title 'Sequence Start' w points\n",result_name_txt.c_str());
    ...
    sprintf(command,"cd %s; gnuplot %s",dir.c_str(),result_name_gp.c_str());
    cout << "here1" << endl;
    system(command);
  }
  ...
  // create pdf and crop
  cout << "here2" << endl;
  sprintf(command,"cd %s; ps2pdf %s.eps %s_large.pdf",dir.c_str(),result_name.c_str(),result_name.c_str());
  system(command);
  sprintf(command,"cd %s; pdfcrop %s_large.pdf %s.pdf",dir.c_str(),result_name.c_str(),result_name.c_str());
  system(command);
  sprintf(command,"cd %s; rm %s_large.pdf",dir.c_str(),result_name.c_str());
  system(command);
}

void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,string result_name) {

  // file names
  char file_name_tl[1024]; sprintf(file_name_tl,"%s/%s_tl.txt",plot_error_dir.c_str(),result_name.c_str());
  char file_name_rl[1024]; sprintf(file_name_rl,"%s/%s_rl.txt",plot_error_dir.c_str(),result_name.c_str());
  char file_name_ts[1024]; sprintf(file_name_ts,"%s/%s_ts.txt",plot_error_dir.c_str(),result_name.c_str());
  char file_name_rs[1024]; sprintf(file_name_rs,"%s/%s_rs.txt",plot_error_dir.c_str(),result_name.c_str());
...
void plotErrorPlots (string dir,string result_name) {
  ...
  for (int32_t i=0; i<4; i++) {
    ...
    sprintf(file_name,"%s_%s.gp",result_name.c_str(),suffix);
    ...
    for (int32_t j=0; j<2; j++) {
      ...
      // plot error curve
      fprintf(fp,"plot \"%s_%s.txt\" using ",result_name.c_str(),suffix);
      switch (i) {
    ...
    // create pdf and crop
    sprintf(command,"cd %s; ps2pdf %s_%s.eps %s_%s_large.pdf",dir.c_str(),result_name.c_str(),suffix,result_name.c_str(),suffix);
    system(command);
    sprintf(command,"cd %s; pdfcrop %s_%s_large.pdf %s_%s.pdf",dir.c_str(),result_name.c_str(),suffix,result_name.c_str(),suffix);
    system(command);
    sprintf(command,"cd %s; rm %s_%s_large.pdf",dir.c_str(),result_name.c_str(),suffix);
    system(command);
  }
}

void saveStats (vector<errors> err,string dir,string seq) {
  ...
  FILE *fp = fopen((dir + "/" + seq + "_stats.txt").c_str(),"w");
...
int training_set_minmax[] = {1,4};

bool eval (string abs_path_to_data) {

  // abs_path_to_data = /media/simone/PortableSSD/datasets/mulran
  // /media/simone/PortableSSD/datasets/mulran/sequences/DCC01/01.txt
  // /media/simone/PortableSSD/datasets/mulran/floam/DCC/DCC01/DCC01_poses_kitti.txt

  // ground truth and result directories
  // abs_path_to_data = /media/simone/PortableSSD/datasets/mulran/
  // string path_to_data = abs_path_to_data + "/sequences";
  string result_dir = abs_path_to_data + "/floam";
  ...
  // for all sequences, for traning 00 to 10, for testing 11 to til 21
  for (int32_t j=0; j<4; j++) {
    ...
    switch (j) {
      ...
      case 1:
        ...
        break;
      case 2:
        sequence_name = "Riverside";
        break;
      case 3:
        sequence_name = "Sejong";
        break;
      default:
        sequence_name = "";
        break;
    }
    for (int32_t i=training_set_minmax[0]; i<training_set_minmax[1]; i++) {
     
      // file name
      char folder_name[256];
      sprintf(folder_name,"%02d",i); // folder name 00, 01, ..

      // string folder_name_str(folder_name); // merda
      // string gt_path = path_to_data + "/" + sequence_name + "" + folder_name_str + "/global_pose_kitti.csv";
      // string estimate_name = "estimate.txt";
      // string estimate_path = path_to_data + "/" + sequence_name + "" + folder_name_str + "/" + estimate_name;

      string folder_name_str(folder_name); // merda
      string gt_path = abs_path_to_data + "/sequences/" + sequence_name + folder_name_str + "/" + folder_name_str + ".txt";
      string estimate_name = sequence_name + folder_name_str + "_poses_kitti.txt";
      string estimate_path = abs_path_to_data + "/floam/" + sequence_name + "/" + sequence_name + folder_name_str + "/" + estimate_name;

      // string gt_path = abs_path_to_data + "/" + folder_name_str + "/gt_kitti.txt";
      // string estimate_name = folder_name_str + "_poses_kitti.txt";
      // string estimate_path = abs_path_to_data +  "/floam/" + folder_name_str + "/" + estimate_name;

      cout << gt_path << endl;
      cout << estimate_path << endl;
      // read ground truth and result poses
      vector<Matrix> poses_gt     = loadPoses(gt_path);
      vector<Matrix> poses_result = loadPoses(estimate_path);

      cout << "Processing: " << estimate_path << ", poses: " << poses_result.size() << "/" << poses_gt.size() << endl; 
      
      // check for errors
      if (poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
        cout << "ERROR: Couldn't read (all) poses of: " << estimate_name << endl;
        continue;
      }

      // compute sequence errors
      string result_name = sequence_name + "_" + folder_name_str + ".txt";
      vector<errors> seq_err = calcSequenceErrors(poses_gt,poses_result);
      saveSequenceErrors(seq_err, error_dir + "/" + result_name);
      cout << "saved" << endl;
      ...
      savePathPlot(poses_gt,poses_result,plot_path_dir + "/" + result_name);
      cout << "saved path" << endl;
      vector<int32_t> roi = computeRoi(poses_gt,poses_result);
      plotPathPlot(plot_path_dir,roi,sequence_name,i);

      // save + plot individual errors
      saveErrorPlots(seq_err,plot_error_dir,result_name);
      plotErrorPlots(plot_error_dir,result_name);

      saveStats(seq_err,result_dir,sequence_name + "_" + folder_name_str);
    }
  }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // we need 3 arguments!
  if (argc!=2) {
    cout << "Usage: ./eval_odometry path_to_data" << endl;
    return 1;
  }

  // read arguments
  string path_to_data = argv[1];
  // string sequence_name = argv[2];

  // run evaluation
  bool success = eval(path_to_data);
